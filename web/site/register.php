...
<body>
	<div id="reg_cont">
		...
		<form action="" method="post" id="reg_form" onsubmit="return false;">
	    	...
	    	<div class="button register"><span class="in_b_1"><p class="in_b_2"><a href="#" id="do_reg" onclick="doReg($('do_reg'), '<?= L::signup_pass_mismatch ?>', '<?= L::login_fields_blank ?>', '<?= L::loading ?>');return false;"><?= L::signup_do_signup ?></a></p></span></div>
	    </form>
	    <div class="button back" style="margin-right:30px;margin-top:10px;"><span class="in_b_1"><p class="in_b_2"><a id="to_main" href="#" onclick="if (!parent.window_loading){ parent.load_window('site/login.php','left'); doLoading($('to_main'), '<?= L::loading ?>');}return false;"><?= L::back ?></a></p></span></div>
