...
class BaseProtocol {
  const BLANK_LOGIN_OR_PASSWORD_ERROR_CODE = 2;
	...
	private static function customLogic($action, $params) {
		...
		switch ($action) {
          ...
          if ($params['login'] == '""' || $params['pass'] == '""') {
            $res['results']['header_result']['error_code'] = self::BLANK_LOGIN_OR_PASSWORD_ERROR_CODE;
          }
					return $res;
				}
      ...
      case 'guest_user_authorize':
        if ($params['name'] == '""') {
          $results["header_result"] = array(
						'success' => 0,
						'error_code' => self::BLANK_LOGIN_OR_PASSWORD_ERROR_CODE,
						'error_params' => null
					);
        } else {
      ...
      case 'get_my_location':
        ...
        if ($res['success']) {
          $res['results']['data_result'] = $res['results']['header_result'];
