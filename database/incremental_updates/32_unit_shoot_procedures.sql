...
BEGIN
  ...
  ELSE
    ...
    ELSE
      IF NOT EXISTS (SELECT id FROM shooting_params sp WHERE sp.unit_id = board_unit_id AND sp.aim_type = aim_type LIMIT 1) THEN
      ...
      ELSE
        ...
        IF distance < (SELECT MIN(distance) FROM shooting_params sp WHERE sp.unit_id = board_unit_id AND sp.aim_type = aim_type) THEN
          SELECT 0 AS `success`, ed.id as `error_code`, null as `error_params` FROM error_dictionary ed WHERE ed.code='target_too_close';
        ELSE
          IF distance > (SELECT MAX(distance) FROM shooting_params sp WHERE sp.unit_id = board_unit_id AND sp.aim_type = aim_type) THEN
          ...
          ELSE
            ...
            IF aim_type = 'unit' THEN
              IF EXISTS (SELECT 1 FROM attack_bonus ab WHERE ab.mode_id=mode_id AND ab.unit_id IS NULL AND ab.aim_type='unit' AND ab.aim_id=aim_board_id) THEN
                SELECT ab.dice_max, ab.chance, ab.damage_bonus INTO dice, chance, damage_modificator FROM attack_bonus ab WHERE ab.mode_id=mode_id AND ab.unit_id IS NULL AND ab.aim_type='unit' AND ab.aim_id=aim_board_id LIMIT 1;
            ...
            IF miss=0 THEN
              SELECT sp.dice_max, sp.chance, get_random_int_between(sp.damage_modificator_min, sp.damage_modificator_max) + damage_modificator
                ...
                FROM shooting_params sp WHERE sp.unit_id = board_unit_id AND sp.aim_type = aim_type AND sp.distance = distance LIMIT 1;
